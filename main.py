...
df_mp = pd.read_csv("qa_response.csv")
qa_data = ""
    ...
    if user_question.lower() == "bye":
        ...
        # calling fun-the linear_predict
            ...
            if filtered_df.empty:
                ...
                result =  f"The predicted {intent} for the next 5 years shows a trend with projected values as follows: {int(fu_years[0])} = {predicted_values[0]}, {int(fu_years[1])} = {predicted_values[1]}, {int(fu_years[2])} = {predicted_values[2]}, {int(fu_years[3])} = {predicted_values[4]}, {int(fu_years[4])} = {predicted_values[4]}. All the financial terms are in millions of USD other than the ratios." 
                return result            
